...
**调用方：** `long-task-st`（Step 12.5，Persist 之后、Verdict 之前）
**依赖：** `feature-list.json` 中 `retro_authorized` = true **且** endpoint 可达
**读取：** `docs/retrospectives/*.md`、`feature-list.json`
**产出：** 将记录移动到 `docs/retrospectives/reported/`
**使用脚本：** `check_retro_auth.py`、`check_retrospective_readiness.py`、`validate_retrospective_record.py`、`post_retrospective_report.py`
