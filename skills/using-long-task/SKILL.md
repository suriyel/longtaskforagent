...
If `repos-manifest.json` exists → **multi-repo project**:
1. Read `repos-manifest.json` → get repo list
2. Check global SRS: does `docs/plans/*-srs.md` exist **at project root**?
   - **NO global SRS** → invoke `long-task:long-task-requirements` (全局需求阶段，在项目根进行)
   ...
   - **Global SRS exists** → check if split is done: do any repos in manifest have SRS in `<repo_path>/docs/plans/*-srs.md`?
     ...
     - **Already split** → proceed to per-repo routing:
       a. Build per-repo status table by scanning each repo directory:
          ...
          For each repo: check `<repo_path>/docs/plans/*-srs.md`, `*-design.md`, `*-ats.md`, `<repo_path>/feature-list.json`
          If has feature-list.json: run `python scripts/check_st_readiness.py <repo_path>/feature-list.json` to determine passing/failing
       b. Present status table to user via `AskUserQuestion`:
          "本次 session 要处理哪个仓库？" — list all repos with their current phase
       c. User selects a repo → set working context to `<repo_path>/`
       d. **Within the selected repo directory**, run normal single-repo Phase Detection (rules 0–7 below)

If `repos-manifest.json` does NOT exist → **single-repo project** → proceed to detection rules below (unchanged).

---

**Detection rules** (single-repo, or within a selected repo in multi-repo):
...
| `repos-manifest.json` | Multi-repo topology manifest — generated by session-start hook when project root has sub-directory git repos (absent in single-repo projects) |
