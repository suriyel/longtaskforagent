...
| # | Check | YES/NO | Evidence |
|---|-------|--------|----------|
| S1 | 文档顶部有必需元数据：Date、Status（必须为"Approved"或"Draft — pending approval"）、Standard 引用（ISO/IEC/IEEE 29148） | | |
| S2 | 全部 11 个模板章节存在（1. Purpose & Scope 到 11. Open Questions）；标记"[Not applicable]"的章节若给出理由则可接受 | | |
| S3 | 第 10 节可追溯矩阵包含文档中定义的每个 FR-xxx 需求 ID — 不可缺少任何需求 | | |
