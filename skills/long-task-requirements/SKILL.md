...
1.1. **[Multi-repo only] Multi-repo context** — explore each repo via `long-task-explore`, identify inter-repo relationships, confirm topology with user
...
15.5. **[Multi-repo only] Split SRS to per-repo** — group FRs by repo, split cross-repo FRs, generate per-repo SRS + IFRs, update repos-manifest.json
16. **Transition to Design** — single-repo: invoke `long-task:long-task-design`; multi-repo: end session (router handles per-repo Design next)

**Single-repo terminal state: invoking long-task-design.** Multi-repo terminal state: session ends after SRS split confirmation.
...
## Step 1.1: Multi-Repo Context (when repos-manifest.json exists)

**Trigger**: `repos-manifest.json` exists in project root (generated by session-start hook).

**Skip if**: `repos-manifest.json` does not exist (single-repo project).
...
2. `repos-manifest.json` exists (multi-repo brownfield) AND the user's description mentions concrete functionality — explore each repo with inferred focus

**Skip if**: greenfield project with no `repos-manifest.json`, OR user description is too vague to derive a focus direction (e.g., "I want to build a platform" with no specifics).
...
3. Dispatch `long-task-explore` with context-driven parameters:
   - **Single-repo or single focus path**:
     ```
     Agent(
       subagent_type="general-purpose",
     ...
     ```
   - **Multi-repo** (repos-manifest.json exists):
     > **Dedup with Step 1.1**: If Step 1.1 already explored a repo, reuse those findings instead of re-dispatching explore for the same repo. Only dispatch explore for repos where the inferred focus dimensions differ significantly from what Step 1.1 used (`architecture,api,deps`), or where Step 1.1 failed/returned no findings.
     
     For repos that need fresh exploration, dispatch in parallel:
     ```
     For each repo in repos-manifest.json (not already covered by Step 1.1):
       Agent(
         subagent_type="general-purpose",
...
#### 8b. Multi-Repo Annotation (when repos-manifest.json exists)
...
This annotation is used in Step 15.5 to split the global SRS into per-repo SRS documents. If a requirement spans multiple repos, it will be split into separate per-repo FRs linked by dependencies and IFR contracts.

**Skip this sub-step** if `repos-manifest.json` does not exist (single-repo project).
...
### Step 15.5: Split Global SRS into Per-Repo SRS (multi-repo only)

**Trigger**: `repos-manifest.json` exists in project root.

**Skip if**: `repos-manifest.json` does not exist (single-repo project). Proceed directly to Step 16.

**Execution**:

1. **Group FRs by repo annotation** (from Step 8b):
   ...
   - Cross-repo FRs → split into per-repo independent FRs:
     ...
     d. For each cross-repo boundary, create an IFR (Interface Requirement) documenting the contract:
        ...
        - Dependency repo absolute path: "Depends on: {absolute_path_to_other_repo}"
...
2. **Generate per-repo SRS documents**:
   ...
   b. Write `<repo_path>/docs/plans/YYYY-MM-DD-<topic>-srs.md`:
      ...
      - Add `## Interface Requirements` section: list all cross-repo IFRs with dependency paths
     ...
     "repos": [
       {
         "name": "backend",
         "path": "backend",
         "srs_path": "backend/docs/plans/2026-04-08-myproject-srs.md",
         "status": "srs-ready"
       },
       {
         "name": "frontend",
         "path": "frontend",
         "srs_path": "frontend/docs/plans/2026-04-08-myproject-srs.md",
         "status": "srs-ready"
       }
     ],
...
**If multi-repo** (`repos-manifest.json` exists):
1. Do NOT invoke `long-task:long-task-design`
2. Inform user: "全局 SRS 已完成并拆分为各仓库独立 SRS。下一个 session 中，路由器会询问您要对哪个仓库开始 Design 阶段。"
3. Update `repos-manifest.json`: set each repo's `status` to `"srs-ready"`
4. End session — the next session's router (Step 0 Multi-Repo Check) will present the repo selection UI
...
**Chains to:** long-task-design (after SRS approval; single-repo) OR ends session (multi-repo — router handles per-repo Design invocation next session)
