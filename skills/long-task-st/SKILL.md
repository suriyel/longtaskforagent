...
- 所有特性都 `"status": "passing"` —— 如有 failing，改为调用 `long-task:long-task-work`
...
**被调用方：** `using-long-task`（feature-list.json 存在且所有特性 passing 时）或 `long-task-work`（Step 12 无 failing 特性剩余时）
**读取：** `feature-list.json`、`docs/plans/*-srs.md`、`docs/plans/*-design.md`、`docs/plans/*-ucd.md`（若 UI）、`docs/test-cases/feature-*.md`（来自 `long-task-feature-st` 的逐特性 ST）、`task-progress.md`、项目 config 文件（如适用）
**可能调用：** `long-task:long-task-work`（发现 Critical/Major 缺陷 → 修复循环）、`long-task:long-task-finalize`（Go/Conditional-Go 裁决后）
**产出：** `docs/plans/YYYY-MM-DD-st-plan.md`、`docs/plans/YYYY-MM-DD-st-report.md`
**按需读取（通过 Read 工具，不是 Skill 工具）：** `references/st-recipes.md`
