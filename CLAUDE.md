...
| Check LCD wiring (cross-doc) | `python scripts/check_lcd_wiring.py feature-list.json docs/plans/*-srs.md` |
...
| `docs/references/*` | 0a (brownfield) / Increment | Archived user-provided original requirement documents (evidence layer; SHA256 logged in SRS §1.4.3). Downstream SubAgents do NOT read by default — referenced only on `[LEGACY-DRIFT]` blocker溯源 |
| `docs/plans/*-srs.md` | 0a / multi-repo | Approved SRS (single-repo: per-repo; multi-repo: global at root + per-repo in each sub-repo). §1.4.2 Legacy Context Decisions (LCD) table = execution authority for legacy semantic constraints (BEHAVIOR / COMPAT / DATA / PERF / RATIONALE categories) |
...
```json
{
  ...
  "srs_trace": ["FR-001"], "lcd_trace": ["LCD-001"], "verification_steps": ["optional scenario"],
  "dependencies": [],
...
- `lcd_trace`: optional; maps feature to SRS §1.4.2 Legacy Context Decisions (ACTIVE non-RATIONALE only). Deep cross-doc check via `scripts/check_lcd_wiring.py`
